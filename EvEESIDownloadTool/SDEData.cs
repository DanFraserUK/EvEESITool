...
namespace EvEESITool
{
    ...
    public class SDEData : IDisposable
    {
        ...
        public SDEData(ref AppSettings settings)
        {
            Settings = settings;
            CheckForData();
        ...
        {
            // These if statements are stacked this way in case of debugging stopping before the next step
            if (Directory.GetFiles(Settings.SDEDirectory, "*.json", SearchOption.TopDirectoryOnly).Length == 0)
            {
                // data doesn't exist
                DownloadData();
                // now we have a lot of jsonQ files
            }
            if (Directory.GetFiles(Settings.SDEDirectory, "*.jsonQ", SearchOption.TopDirectoryOnly).Length > 0)
            {
                // data does exist, but not in the correct format we need
                List<AgtAgents> agtAgents = new List<AgtAgents>();
        ...
        public T GetSDEData<T>(string fileName)
        {
            ...
            {
                if (Directory.GetFiles(Settings.SDEDirectory, "*", SearchOption.TopDirectoryOnly).Length == 0)
                {
                    SDEJson.PullData();
                }
                else
                {
                    ...
                    files.AddRange(Directory.GetFiles(Settings.SDEDirectory, "*.jsonQ", SearchOption.TopDirectoryOnly));
                    foreach (string s in files)
                    {
                        List<SDEDataBaseClass> test = new List<SDEDataBaseClass>();
                        string input;
                        using (StreamReader myReader = new StreamReader(s))
                        {
                            input = myReader.ReadToEnd();
                        }
                        test = JsonConvert.DeserializeObject<List<SDEDataBaseClass>>(input);
                        string testString = test[18].Lines();
        ...
        private static class SDEJson
        {
            ...
            static List<SDEJsonData> Data = new List<SDEJsonData>();

            public static void PullData()
            {
                var a = PullJsonList("http://sde.zzeve.com/tables.json");

                while (!a.IsCompleted)
                {
                    Task.Delay(50).Wait();
                }
                Data = JsonConvert.DeserializeObject<List<SDEJsonData>>(a.Result);
                ProcessData();
            }

            private static void ProcessData()
            {
                foreach (SDEJsonData s in Data)
                {
                    WriteJsonToFile(s);
                }
            }

            private static void WriteJsonToFile(SDEJsonData s)
            {
                var b = PullJsonList(s.Href);
                List<object> data = new List<object>();
                ...
                data = JsonConvert.DeserializeObject<List<object>>(b.Result);
                SaveJsonDataToFile(s, data);
            }

            private static void SaveJsonDataToFile(SDEJsonData s, List<object> data)
            {
                string currentPath = Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().CodeBase).Substring(6);
                if (!Directory.Exists(currentPath + @"\SDE\"))
                {
                ...
                using (StreamWriter myWriter = new StreamWriter(currentPath + @"\SDE\" + s.Href.Substring(s.Href.LastIndexOf('/') + 1)))
                {
                    ...
                    {
                        AppSettings.serializer.Serialize(writer, data);
                    }
                }
            }

            private static async Task<string> PullJsonList(string link)
            {
                return await client.GetStringAsync(link);
            }
        }
        private class SDEJsonData
        {
            [JsonProperty("name")]
